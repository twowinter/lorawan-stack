...
func newMACState(dev *ttnpb.EndDevice, fps *frequencyplans.Store, defaults ttnpb.MACSettings) (*ttnpb.MACState, error) {
	...
	if dev.SupportsClassC {
		st.DeviceClass = ttnpb.CLASS_C
	} else if dev.SupportsClassB {
		st.DeviceClass = ttnpb.CLASS_B
	} else {
		st.DeviceClass = ttnpb.CLASS_A
	}

	st.CurrentParameters.MaxEIRP = band.DefaultMaxEIRP
	...
	st.CurrentParameters.ADRAckLimit = uint32(band.ADRAckLimit)
	st.DesiredParameters.ADRAckLimit = st.CurrentParameters.ADRAckLimit

	st.CurrentParameters.ADRAckDelay = uint32(band.ADRAckDelay)
	st.DesiredParameters.ADRAckDelay = st.CurrentParameters.ADRAckDelay

	st.CurrentParameters.Rx1Delay = ttnpb.RxDelay(band.ReceiveDelay1.Seconds())
	...
	st.CurrentParameters.Rx2DataRateIndex = band.DefaultRx2Parameters.DataRateIndex
	...
	st.CurrentParameters.Rx2Frequency = band.DefaultRx2Parameters.Frequency
	...
	if len(dev.GetMACSettings().GetFactoryPresetFrequencies()) > 0 {
		...
		st.CurrentParameters.Channels = make([]*ttnpb.MACParameters_Channel, 0, len(band.UplinkChannels))
		...
		for i, downCh := range band.DownlinkChannels {
			st.CurrentParameters.Channels[i].DownlinkFrequency = downCh.Frequency
		}
	}

	st.DesiredParameters.Channels = make([]*ttnpb.MACParameters_Channel, 0, len(band.UplinkChannels)+len(fp.UplinkChannels))
	for _, upCh := range band.UplinkChannels {
	...
	for i, downCh := range band.DownlinkChannels {
		st.DesiredParameters.Channels[i].DownlinkFrequency = downCh.Frequency
	}
